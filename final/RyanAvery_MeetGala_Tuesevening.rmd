...
```{r}
library(dplyr)
county_winner2 = election_county %>% 
...
It appears that Gender (Men), employment related variables (Employed, Office, and PrivateWork), and the proportion of the county that are citizens (Citizen), explained most of the variance in the predictors by geography. This is true for both the county level and the subcounty level.
...
subct.pc = as.data.frame(subct.out$rotation[,1:2])
ct.pc = as.data.frame(ct.out$rotation[,1:2])
kable(tail(sort(abs(subct.pc)),6), col.names = "Sub County Highest Loadings")

kable(tail(sort(abs(ct.pc)),6), col.names = "County Highest Loadings")

```


NEED TO MAKE PRETTY TABLES with kable_Styling?
...
```{r}
census.ct.dist = dist(select(ungroup(census.ct),-one_of(c("State","County"))))
...
```{r}
# Plot dendogram
plot(census.ct.hclust, labels=census.ct$County, main='Default from hclust')
...
```{r}
library(lattice)
levelplot(as.matrix(census.ct.dist)[census.ct.hclust$order,
                                          census.ct.hclust$order],at=pretty(c(0, 100), n=10),scales=list(y=list(cex=.5), x=list(cex=.5)))
```

```{r}
groups.10 = cutree(census.ct.hclust,10)
census.ct$group = groups.10
sanmateo_group = filter(census.ct, group==1)
...
# maybe helpful: https://stackoverflow.com/questions/23714052/ggplot-mapping-us-counties-problems-with-visualization-shapes-in-r
counties = map_data("county")
counties = left_join(counties,ungroup(sanmateo_group),by=c('region','subregion'))

    
ggplot(data = counties) + 
  geom_polygon(aes(x = long, y = lat, fill = Black, group = group.x), color = "grey", size = .1) + 
  coord_fixed(1.3) +
  guides(fill="colorbar") +
  ggtitle('Percenteage of Population that is Category by County')
